...
class VisualiseCARV1(object):
    ...
    def get_data(self):
        ...
        b = bagreader(self.data_path + '/' + self.data_name)
        data_gps = b.message_by_topic('/car_client/get_gps')
        print("File saved: {}".format(data_gps))
        df_gps = pd.read_csv(data_gps)

        for msg in df_gps.data:
            mesg = json.loads(msg)
            self.v1_gps_x.append(round(mesg["lat"] * 111139, 1))
            self.v1_gps_y.append(round(mesg["long"] * 111139, 1))

    def plot_ht(self):
        v1_max_x = round(max(self.df_gps_x))
        v1_min_x = round(min(self.df_gps_x))
        v1_max_y = round(max(self.df_gps_y))
        v1_min_y = round(min(self.df_gps_y))

        data_status = np.zeros((v1_max_x - v1_min_x,
                                v1_max_y - v1_min_y))

        for i, x in enumerate(self.v1_gps_x):
            for j, y in enumerate(self.v1_gps_y):
                if i == j:
                    data_status[round(x) - v1_min_x - 1, round(y) - v1_min_y - 1] = self.STATE
                    break

        df_ht = pd.DataFrame(data_status,
                             index=np.linspace(v1_min_x,
                                               v1_max_x - 1,
                                               v1_max_x - v1_min_x,
                                               dtype='int'),
                             columns=np.linspace(v1_min_y,
                                                 v1_max_y - 1,
                                                 v1_max_y - v1_min_y,
                                                 dtype='int'))
        ...
        myColors = ((0.9, 0.48, 0.0, 1.0), (0.0, 0.0, 0.8, 1.0))
        ...
        colorbar.set_ticks([-135, -120])
        colorbar.set_ticklabels(["V1", "Robot"])
        ...
        self.fig.savefig(self.data_path + "/fig/V1_Signal_Heatmap" + ".pdf")
